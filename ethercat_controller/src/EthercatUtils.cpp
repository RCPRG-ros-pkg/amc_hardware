...
void printErrors()
{
	...
	{
		errors = true;
		printf("error: slave=%d index=%.4X.%.2Xh etype=%d errorcode=%d abortcode=0x%X\n", (int)err.Slave, (int)err.Index, (int)err.SubIdx, (int)err.Etype, (int)err.ErrorCode, (int)err.AbortCode);
	}
}
...
char* SDO2string(uint16 slave, uint16 index, uint8 subidx, uint16 dtype)
{
   ...
   else
   {
      switch(dtype)
      {
         ...
         case ECT_INTEGER64:
            i64 = (int64*) &usdo[0];
            sprintf(hstr, "0x%16.16llx %lld", (uint64)*i64, *i64); 
            break;
         case ECT_UNSIGNED8:
            u8 = (uint8*) &usdo[0];
...
bool ethercatInit(const char *ifname_in)
{
	int cnt, i, j, nSM;
	uint16 ssigen;
	int expectedWKC;

	// ec_init takes char*, not const char*
	char *ifname = new char[strlen(ifname_in)+1];
	memcpy(ifname, ifname_in, strlen(ifname_in)+1);
...
//		printf("ec_init on %s succeeded.\n",ifname);
		...
		if ( ec_config(FALSE, &IOmap) > 0 )
		{
			if (!ec_configdc())
			{
				printf("ERROR: ec_configdc\n");
			}
			else
				printf("ec_configdc OK\n");
//			while(EcatError) printf("%s", ec_elist2string());
...
//			printf("Calculated workcounter %d\n", expectedWKC);
			...
			// ec_slave[0] is reserved for the master
...
bool ethercatEnterSafeOpState()
{
	...
	ec_writestate(0);
	return EC_STATE_SAFE_OP == ec_statecheck(0, EC_STATE_SAFE_OP, EC_TIMEOUTSTATE);
}
...
bool ethercatEnterOperationalState()
{
	...
	ec_writestate(0);
	return EC_STATE_OPERATIONAL == ec_statecheck(0, EC_STATE_OPERATIONAL, EC_TIMEOUTSTATE);
}
...
bool ethercatEnterOperationalStateNoBlock()
{
	ec_slave[0].state = EC_STATE_OPERATIONAL;
	ec_writestate(0);
	return EC_STATE_OPERATIONAL == ec_statecheck(0, EC_STATE_OPERATIONAL, 0);
}
